...
export default function Post( props ) {
    ...
    const addCommentHandler = async () => {
        let response = await fetch(`/api/comments/${post.id}/new`, {
            method: 'POST',
            headers: {
    ...
    const likeHandler = async () => {
        let response = await fetch(`/api/posts/${post.id}/like`, {
            headers: {
                Authorization: auth.username
            }
        })
    ...
    useEffect(() => {
        const fetchProfilePic = async () => {
            const response = await fetch(`/api/users/u/${post.creatorUsername}`)
    ...
    useEffect(() => {
        const fetchPostLikes = async () => {
            let response = await fetch(`/api/posts/${post.id}/likes`)
            let responseData = await response.json()
            if(response.status === 200)
    ...
    useEffect( () =>{
        const fetchComments = async () => {
            let response = await fetch(`/api/comments/${post.id}`)
            let responseData = await response.json()
            if(response.status === 200)
